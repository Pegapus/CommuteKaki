...
The Point-to-Point planner uses OneMap Search for building-level Singapore postcodes and named MRT stations, including autocomplete. Both selected suggestions and typed searches use these coordinates: a general postcode or neighbourhood centre can connect to a different walking path and produce large detours. The resolved address is shown in the input before routing.

The planner asks OneMap for up to three itineraries across nine transit profiles (bus, rail, and mixed transit with 500 m, 1 km, and 2 km walking limits). It removes duplicate paths, adds current LTA rail crowd readings, and ranks the remaining alternatives by fastest journey, crowd level, number of transfers, or walking distance. Optional limits let commuters exclude crowded, transfer-heavy, or long-walking routes. Walking legs use a grey dotted line on the map.
...
Create an ignored `.env` file and fill in `MONGODB_URI`, `MONGODB_DBNAME`, `MAPBOX_API_KEY`, `LTA_DATAMALL_KEY`, `ONEMAP_TOKEN` (or `ONEMAP_API_KEY`), `FRONT_END_URL`, and `BRIDGING_POINTS_SHEET_ID`.
...
| `GET` | `/api/places?q=` | OneMap Singapore address and station suggestions, with exact postcode matching |
| `GET` | `/api/routing/plan?fromPlace=&toPlace=&date=&time=&preference=&maxCrowd=&maxTransfers=&maxWalk=&includeUnknown=` | Returns deduplicated and ranked OneMap point-to-point alternatives, enriched with LTA rail crowd readings |
